...
ConfigurationParser::Error::operator ConfigurationParser::ErrorType() const {
    return type;
}
...
ConfigurationParser::Error ConfigurationParser::parse(Configuration &output, const char *jsonString) {
    ...
    return Error(ErrorType::OK, static_cast<int>(parser.cur-jsonString));
}

void ConfigurationParser::parseStdString(std::string &value) {
    skipWhitespace();
    if (*cur != '"')
        throw ErrorType::STRING_EXPECTED;
    value.clear();
    ++cur;
    while (*cur != '"') {
        if (*cur == '\\') {
...
void ConfigurationParser::parseSettingsJsonIO(Settings::JsonIO &value) {
    ...
    if (str == "NULL_TERMINATED_STRING")
        ...
        throw ErrorType::UNKNOWN_ENUM_VALUE;
}
